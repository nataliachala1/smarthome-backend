...
@Injectable()
export class PrismaDeviceTypeRepository
  implements DeviceTypeRepository
{
  constructor(
  ...
  async findAll(): Promise<DeviceType[]> {
    const rows =
      await this.prisma.device_type.findMany({
        where: {
          deleted_at: null,
        },
    ...
    return rows.map((row) =>
      PrismaDeviceTypeMapper.toDomain(
        row,
      ),
    );
  }
  ...
  async findById(
    ...
    const raw =
      await this.prisma.device_type.findFirst({
        where: {
          id_device_type: id,
          deleted_at: null,
        },
