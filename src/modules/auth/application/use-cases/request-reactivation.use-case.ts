...
export interface RequestReactivationOutput {
  sent: boolean;
}
...
export class RequestReactivationUseCase {
  ...
  async execute(
     ...
     * si una cuenta existe o está desactivada.
     */
    if (
      !user ||
      user.status !==
    ...
    const reactivationUrl =
      `${frontendUrl}/reactivate-account?token=${encodeURIComponent(token)}`;

    await this.emailSender.sendAccountReactivationEmail({
      to:
        user.email,
